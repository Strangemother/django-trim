...
urlpatterns = trim.paths_dict(views, dict(
        VacgenListView=('list', ''),
        ...
        AttributeTitleValueListView=('attributetitle-value-list',
                                     'filter/<str:title_slug>/<str:pk>/'),

        ProductStashListView=('productstash-list', 'stash/'),
        ProductStashLabelFormView=('productstash-label-form', 'stash/form/<str:uuid>/label/'),
        ProductStashMergeFormView=('productstash-merge-form', 'stash/form/<str:uuid>/merge/'),
        ProductStashFormView=('productstash-form', ('stash/form/', 'stash/form/<str:uuid>/')),
        ProductStashDetailView=('productstash-detail', 'stash/<str:uuid>/'),

        # This URL should exist before the detail/<str>/
        ProductContactSuccessView=('contact-success', 'contact/<str:uuid>/',),
        ProductContactFormView=('contact-form', 'contact/',),
        ## If 'detail' fails, the view serves 'filter'.
        ProductStockNotifyListView=('stock-notify-list', '<str:product_id>/stock-notify-list/' ),
        ...
        VacgenProductProductAttributeValuesListView=('productattribute', 'attribute/<str:slug>/'),
        VacgenProductFilterView=('filter','filter/<str:product_id>/'),
        ImageDetail=('image-detail', 'graphic/<str:pk>/',),
        ImageDetailFullSize=('fullsize-image-detail', 'graphic/<str:pk>/large/',),
...
class ClassPaths(trim.Paths):
    ...
    ## Filter the list of products within an attributevalue list,
    ## using the inherited title; e.g. spares/yes  | power/yes
    AttributeTitleValueListView=('attributetitle-value-list', 'filter/<str:title_slug>/<str:pk>/')
    ProductStashListView=('productstash-list', 'stash/')
    ProductStashLabelFormView=('productstash-label-form', 'stash/form/<str:uuid>/label/')
    ProductStashMergeFormView=('productstash-merge-form', 'stash/form/<str:uuid>/merge/')
    ProductStashFormView=('productstash-form', ('stash/form/', 'stash/form/<str:uuid>/',))
