...
class OrderPaginatedListView(ListView):
    ...
    def get_selected_orderby(self):
        return (
            self.filter_data.get(self.order_by_field, None)
            or self.default_selected_orderby
        )

    def get_selected_direction(self):
        return (
            self.filter_data.get(self.direction_field, None) or self.default_direction
        )
    ...
    def format_internal(self, data):
        ...
        user_pagination = self.clamp_paginate_by(
            data.get(self.paginate_by_field, None) or None
        )
