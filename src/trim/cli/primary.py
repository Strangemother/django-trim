...
class Help(BHelp):
    double_dash = (
        "apply a double-switch before the " "input to ensure switch aggregation."
    )
...
class GraphApps(object):
    ...
    def build_graph_parsers(self, top, parser=None, depth=0, position=None):
        ...
        for key in top.get("keys", ()):
            ...
            sub_parser = self.add_sub(
                func, key, parser, formatter_class=SubHelpFormatter
            )
            # recurse.
            self.build_graph_parsers(
                data, sub_parser, depth=depth + 1, position=position
            )
...
class ScriptsAddFilenameArg(AppArgument):
    ...
    def setup_args(self, parser):
        parser.add_argument(
            "filename", type=str, nargs="*", help=Help.add_filename, default="."
        )
        parser.add_argument(
            "--", dest="_", nargs="+", type=str, default=None, help=Help.double_dash
        )
        parser.add_argument(
            "--type",
            type=str,
        ...
        parser.add_argument(
            "-pa", "--pattern-append", type=str, default=None, help=Help.pattern_append
        )
